...
const ThreadList = forwardRef(({ currentThreadId, onThreadSelect }: ThreadListProps, ref) => {
  ...
  useEffect(() => {
    const fetchThreads = async () => {
      const response = await fetch('/api/assistants/threads/history');
      const threadIds = await response.json();
      setThreads(threadIds.reverse()); // 最新的对话在前面
    };
    fetchThreads();
  }, []);
  ...
  return (
    <div className={styles.sidebar}>
      ...
      {threads.map((threadId) => (
        <div
          key={threadId}
          className={`${styles.threadItem} ${threadId === currentThreadId ? styles.active : ''}`}
          onClick={() => onThreadSelect(threadId)}
        >
          <span>{threadId}</span>
          <button 
            className={styles.deleteBtn}
            onClick={(e) => deleteThread(threadId, e)}
          >
            <TrashIcon />
          </button>
        </div>
