...
const FolderIcon = () => (
  ...
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M4.5 1.5h-3c-.825 0-1.5.675-1.5 1.5v6c0 .825.675 1.5 1.5 1.5h9c.825 0 1.5-.675 1.5-1.5v-4.5c0-.825-.675-1.5-1.5-1.5h-4.5l-1.5-1.5zm-3 1.5h3l1.5 1.5h4.5v4.5h-9v-6z"
    />
  </svg>
);
...
const OutputFileManager = () => {
  const [outputFiles, setOutputFiles] = useState<string[]>([]);

  const fetchFiles = async () => {
    try {
      const res = await fetch("/api/file-manage");
  ...
  useEffect(() => {
    ...
    // Set up a timer to fetch files every 2 seconds
  ...
  const handleOpenFolder = async (filename: string) => {
    try {
      const res = await fetch(`/api/file-manage/open?filename=${filename}`);
      if (!res.ok) {
  ...
  const handleFileDelete = async (filename: string) => {
    try {
      const res = await fetch(`/api/file-manage/${filename}`, {
        method: "DELETE",
      });
      if (!res.ok) {
...
export default OutputFileManager;